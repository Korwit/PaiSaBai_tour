          ...
          return (
            <Card key={e.id} className="paymentstatus-main">
              ...
              <CardBody className="B4">
                <CardTitle className="B5" style={{ fontSize: "20px" }}>
                  {e.attributes.tour?.data?.attributes?.name
                    ? e.attributes.tour.data.attributes.name
                    : ""}
                </CardTitle>
                <CardText className="B6">
                  ผู้จอง :{" "}
                  <Badge>
                    {e.attributes.owner?.data?.attributes?.firstname
                      ? e.attributes.owner.data.attributes.firstname
                      : ""}
                    {e.attributes.owner?.data?.attributes?.lastname
                      ? e.attributes.owner.data.attributes.lastname
                      : ""}
                  </Badge>
                  <br />
                  วันที่ชำระเงิน:{" "}
                  {e.attributes.payment_date &&
                    new Date(e.attributes.payment_date)
                  ...
                  <br />
                  เวลาชำระเงิน:{" "}
                  {e.attributes.payment_time &&
                    e.attributes.payment_time.slice(0, 5).replace(":", ".")}
                  <br />
                  ราคา:{" "}
                  {e.attributes.tour?.data?.attributes?.price
                    ? e.attributes.tour.data.attributes.price.toLocaleString()
                    : ""}{" "}
                  บาท
                  <Form>
                    <Form.Check
